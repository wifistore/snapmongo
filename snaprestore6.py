...
def main():
  ...
  for hostname in shardnames:
    ...
    if (mm >= 16 and mm <= 18):
      ...
      if (bksz > 100.0):
        print 'please clear previous space 6 '
        return
      ...
      rpath = restore_path+"6/"
      hostnpath.append((scptarget[5],rpath))
      ressz1 = checkspacedfR(scptarget[5],rpath)
      print ' Restore size ' + str(ressz1)
      if (bksz1 > ressz1):
        print 'please clear Restore space 6'
        return
      #scpdump(hostname,shardpath+dbname,scptarget[5],rpath)
    elif (mm >= 1 and mm <= 3 ):
    ...
    elif (mm >= 13 and mm <= 15):
      shardpath = shard_data_path+"5/"
      print shardpath
      bksz = checkspacedf(hostname,shardpath)
      if (bksz < umsmax):
        print '/backup size too small'
      ...
      bksz = checkspace(hostname,shardpath)
      if (bksz > 100.0):
        print 'please clear previous space 5'
        return
      #mngdump(hostname,shardpath)
      #chmodmngdump(hostname,shardpath)
      bksz1 = checkspace(hostname,shardpath)
      print ' After dump snap size ' + str(bksz1)
      #scp to a consolidated node, check size first
      rpath = restore_path+"5/"
      hostnpath.append((scptarget[4],rpath))
      ressz1 = checkspacedfR(scptarget[4],rpath)
      print ' Restore size ' + str(ressz1)
      if (bksz1 > ressz1):
        print 'please clear Restore space 5'
      #scpdump(hostname,shardpath+dbname,scptarget[4],rpath)
